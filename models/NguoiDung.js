...
class NguoiDung {
    ...
    static updateuser = async(ho_ten,gioi_tinh,ngay_sinh,so_dien_thoai,ma)=>
    {
        const sql = 'UPDATE nguoidung SET ho_ten = ? , gioi_tinh = ? , so_dien_thoai = ?  , ngay_sinh = ? WHERE ma_nguoi_dung = ? ';
        const [result] = await db.query(sql,[ho_ten,gioi_tinh,so_dien_thoai,ngay_sinh,ma]);
        return result;
        
    };

    static regeister = async (ho_ten,email,mat_khau,gioi_tinh)=>
    {
        const pass = await bcrypt.hash(mat_khau,10);
        const sql = 'INSERT INTO nguoidung (ho_ten,email,mat_khau,gioi_tinh) VALUES (?,?,?,?)';
        const [result] = await db.query(sql,[ho_ten,email,pass,gioi_tinh]);
        return {id:result.insertId, ho_ten,email,gioi_tinh};
    };
